...
cargo bundle --release -p pdf-editor
echo ""
...
EXECUTABLE="$BUNDLE_DIR/pdf-editor"
...
if [ -f "$BUNDLE_DIR/pdf-editor" ] && [ -f "$BUNDLE_DIR/libpdfium.dylib" ]; then
    echo "✓ Bundle is complete!"
    echo ""
    echo "App location:"
    echo "  $PROJECT_DIR/target/release/bundle/osx/PDF Editor.app"
    echo ""
    echo "To run:"
    echo "  open \"$PROJECT_DIR/target/release/bundle/osx/PDF Editor.app\""
    echo ""
    echo "To install:"
    echo "  cp -r \"$PROJECT_DIR/target/release/bundle/osx/PDF Editor.app\" /Applications/"
else
